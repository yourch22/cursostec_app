@extends('layouts.app')
@section('_title', '⭐ '.$categoriadetalle[0]['categoria'].':')  
@section('meta_title', 'Los mejores cursos de '.$categoriadetalle[0]['categoria'])
...
@section('content')
    ...
    <div
        class="w-screen sm:w-full md:full lg:w-full xl:w-full mx-auto grid grid-cols-1 px-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-10">
        ...
        @endif
            <div class="c-card block bg-white shadow-md hover:shadow-xl rounded-lg overflow-hidden">
                {{-- <a href="{{ config('services.cred_affiliate_admitad.url') . '' . config('services.cred_affiliate_admitad.sub_id') . config('services.cred_affiliate_admitad.url_udemy') . $item->url }}"
                    target="_blank" class="c-card block bg-white shadow-md hover:shadow-xl rounded-lg overflow-hidden"> --}}
                    <div class="relative pb-48 overflow-hidden">
                        <img class="absolute inset-0 h-full w-full object-cover" src="{{ $item->image_240x135 }}" alt="">
                    </div>
                    <div class="p-4">
                        <h2 class="mt-2 mb-2  font-bold">{{ $item->title }}</h2>
                        <p class="text-sm">{{ __('Creado por:') }} <i>{{ $item->visible_instructors[0]->display_name }}</i>
                        </p>
                        <div class="mt-2 flex items-center text-sm text-gray-600">
                            <span
                                class="inline-block text-white bg-yellow-400 font-bold text-sm py-0 px-1 rounded">{{ round($item->avg_rating, 1) }}</span>
                            @php
                        ...
                        <div class="grid grid-cols-2">
                            <div class="col-span-2 text-center">
                                ...
                                <form action="{{ route('udemy_gratis',['rutafree'=>$item->published_title]) }}" method="post">
                                    ...
                                    {{-- <button  target="_blank" class="bg-blue-850 text-sm hover:bg-blue-750 text-white font-bold py-2 px-6 rounded-full inline-flex items-center"> --}}
                                    
                                    <button type="submit"  target="_blank"
                                        class="bg-blue-850 text-sm hover:bg-blue-750 text-white font-bold py-2 px-6 rounded-full inline-flex items-center">
                                        <span>{{ __('Ir al Curso') }}</span>
                                        <svg class="fill-current w-4 h-4 ml-2" version="1.1" id="Capa_1"
                                ...
                                @else
                                    <button href="https://www.udemy.com{{ $item->url }}" target="_blank"
                                        class="bg-blue-850 text-sm hover:bg-blue-750 text-white font-bold py-2 px-6 rounded-full inline-flex items-center">
    ...
    <div class="container mx-auto items-center content-center text-center">
        <div class="bg-white py-5 content-center justify-between border-t border-gray-200">
            <nav id="uk-pagination" class="relative z-0 content-center inline shadow-sm" lan="{{ app()->getLocale() }}"
                url="{{ url('/courses') }}" pagina="{{ $page }}" tipo="category"
                totalpage="{{ $totalpaginas }}" cat="{{ $categoriadetalle[0]['ruta'] }}">
            </nav>
        </div>
    </div>
@endsection
